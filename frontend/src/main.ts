...
function showWeather(data: WeatherResponse): void {
  ...
  document.getElementById('wFeels')!.textContent          = `${Math.round(data.feels_like)}°C`
  ...
  document.getElementById('wTime')!.textContent           = `Consultado el ${formatDate(data.queried_at)}`
